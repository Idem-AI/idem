...
@Component({
  ...
  template: `
    <h1 class="mb-6 text-2xl font-bold">Add server</h1>
    <form class="box max-w-lg space-y-4" [formGroup]="form" (ngSubmit)="submit()">
      ...
      <div>
        <label class="mb-1 block text-sm">IP address</label>
        <input class="input" formControlName="ip" />
      </div>
      <div class="flex gap-4">
        <div class="flex-1">
          <label class="mb-1 block text-sm">SSH user</label>
      ...
      <div>
        <label class="mb-1 block text-sm">Private key ID</label>
...
export class ServerCreateComponent {
  ...
  protected submit(): void {
    ...
    this.api.createServer(this.form.getRawValue()).subscribe({
      ...
      error: (e) => {
        this.error.set(e?.error?.error?.message ?? 'Failed to create server');
