...
export const generateDiagramController = async (
  ...
  try {
    ...
    res.status(201).json(newDiagram || { message: 'Diagram generated but not returned' });
  } catch (error: any) {
    logger.error(
...
export const updateDiagramController = async (req: CustomRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (updatedProject) {
      ...
      res.status(200).json(updatedDiagram || { message: 'Diagram updated but not returned' });
    } else {
...
export const generateDiagramStreamingController = async (
  ...
  try {
    if (!userId) {
      logger.warn('User not authenticated for generateDiagramStreamingController');
      res.status(401).json({ message: 'User not authenticated' });
      return;
    }

    if (!projectId) {
      logger.warn('Project ID is required for generateDiagramStreamingController');
      res.status(400).json({ message: 'Project ID is required' });
      return;
    }
    ...
    if (!updatedProject) {
      logger.warn(`Failed to generate diagram - UserId: ${userId}, ProjectId: ${projectId}`);
      res.write(`data: ${JSON.stringify({ error: 'Failed to generate diagram' })}\n\n`);
      res.end();
      return;
    }
    ...
    res.write(`data: ${JSON.stringify({ type: 'complete', diagram: newDiagram })}\n\n`);
    res.end();
  } catch (error: any) {
    logger.error(
...
export const deleteDiagramController = async (req: CustomRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (success) {
      ...
      logger.warn(`Diagram not found for deletion - UserId: ${userId}, DiagramId: ${diagramId}`);
