/**
 * Mathématiques de couleur du design system calculé.
 *
 * ⚠️ PORT de `apps/appgen/apps/we-dev-next/src/design/color.ts`. Les deux copies
 * doivent rester identiques : c'est la même conversion OKLab, le même calcul de
 * contraste WCAG et les mêmes rampes. Le jour où un `packages/design-system`
 * existe, ce fichier disparaît au profit de lui — l'extraction est un
 * déplacement, pas une réécriture.
 *
 * Tout est déterministe et sans dépendance, et c'est le point : un design system
 * CALCULÉ ne peut pas revenir à 3,1:1 parce que le modèle a trouvé le gris clair
 * élégant. C'est la différence entre une qualité espérée et une qualité garantie
 * — la seule qui tienne quand le modèle rapetisse.
 *
 * Conversions d'après la spécification OKLab de Björn Ottosson.
 */
