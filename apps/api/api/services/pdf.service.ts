...
export class PdfService {
  ...
  private generateOptimizedHtmlFromSections(options: {
      ...
      <head>
        ...
        <style>
          * {
          ...
          /* Une page à hauteur fixe ne porte rien en position FIXE : à
             l'impression, un élément fixe se répète sur chaque page du PDF. */
          .section .fixed,
          .section [style*="position:fixed"],
          .section [style*="position: fixed"] { position: absolute !important; }`
          }
    ...
    sections.forEach((section) => {
      ...
      if (!multiPage) {
        // Chaque page est refermée SEULE avant d'être posée : une section qui
        // oublie de fermer ses conteneurs ne doit pas avaler les suivantes —
        // c'est ainsi que la signature finale s'est retrouvée coincée dans la
        // page de direction artistique (cf. `balanceHtmlFragment`).
        htmlContent += `
        <div class="section" data-section-name="${escapeAttr(section.name || '')}">
            <div class="data-content">${balanceHtmlFragment(sectionData)}</div>
        </div>
      `;
        return;
      }
    ...
    //
    // Elle est SEULE sur sa page : les sections qui précèdent ont été
    // refermées une à une (cf. `balanceHtmlFragment`), aucune ne peut donc
    // plus l'envelopper.
    htmlContent += `
          <div class="${multiPage ? 'idem-page' : 'section'} idem-signature" data-section-name="Generated by Idem">
