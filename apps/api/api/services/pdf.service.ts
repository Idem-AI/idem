...
export class PdfService {
  ...
  private generateOptimizedHtmlFromSections(options: {
      ...
      <head>
        ...
        <style>
          * {
          ...
          ${
            multiPage
              ? `/* Flexible document (business plan): a section MAY span several
             pages. min-height + visible overflow so nothing is clipped, and
             break-inside rules keep blocks whole across page boundaries. */
          .section {
            display: block;
            width: ${format.width};
          ...
          .data-content { width: 100%; height: auto; }
          /* Avoid splitting atomic blocks across pages (tables, images, charts).
             Deliberately narrow: over-broad selectors (every card/border) break
             pagination and leave large empty gaps. */
          .data-content table,
          .data-content thead,
          .data-content tr,
          .data-content img,
          ...
          @media print {
            .section {
              ...
              ${multiPage ? 'overflow: visible !important;' : `max-height: ${format.height} !important; overflow: hidden !important;`}
            }
    ...
    sections.forEach((section, index) => {
      ...
      // Business plan (multi-page) uniquement : sur le CONTENEUR RACINE seul
      // (1er élément avec class=), on remplace la hauteur fixe par min-h et on
      // enlève overflow-hidden pour que le contenu s'étende sur plusieurs pages.
      // On NE touche PAS aux classes internes (sinon images/hauteurs déréglées).
      if (multiPage) {
