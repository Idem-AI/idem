...
Initials-only logo (monogram). Letters are the mark.

LETTER RULES:
- Max 2 initials (preferred) or 3 initials maximum. No full words. UPPERCASE unless specified.

CONTAINERS (Choose one based on archetype):
...
- custom_shape: shape derived from letters.
Container is ALWAYS centered on (40, 40).

CONSTRUCTION GRID (80x80 ViewBox):
- viewBox: "0 0 80 80", u = 10, snap to u/2 (5).
- Letter zone: 56x56 centered at (40, 40). Padding ≥ 12px.
- 2 letters: optical centers at (26.5, 40) and (53.5, 40) (axial_vertical symmetry).
...
- Colored container -> white letters; light container -> colored letters.

ADVANCED TECHNIQUES (Apply exactly ONE):
- OVERLAP: overlap zones use opacity 0.85 (no filters).
- CUTOUT: use fill-rule="evenodd" (reversed inner path direction, no background-colored shapes).
- WEIGHT CONTRAST: one letter Bold (700+), the other Light (300).
- COLOR SPLIT: each letter in different brand color (equal luminance).
- ROTATION: rotate one letter ±5° and balance other letter.
- MONOGRAM LOCK: letters share one stroke joint on symmetry axis (x=40).
