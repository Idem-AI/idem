...
export const AGENT_SOURCE = [
  ...
  '    if (!enabled || editing || !selectedEl) return;',
  '    if (e.key === "Delete"){',
  '      e.preventDefault(); e.stopPropagation();',
  '      var id = selectedEl.getAttribute(ATTR);',
  '      selBox.style.display = "none"; hoverBox.style.display = "none";',
  '      selectedEl = null;',
  '      post({ type: "DELETE_ELEMENT", id: id });',
  '    }',
  '  }',
  '',
