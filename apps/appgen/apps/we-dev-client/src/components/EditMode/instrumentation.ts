...
export const AGENT_SOURCE = [
  ...
  '  var enabled = false, editing = false, suppressClick = false, mode = "select";',
  '  var lastX = -999, lastY = -999, cycleIndex = 0, pendingStack = [];',
  '  var selectedEls = [];',
  ...
  '  }',
  '',
  '  function infoLite(el){ return { id: el.getAttribute(ATTR), tag: el.tagName.toLowerCase(), kind: classify(el) }; }',
  '  /* Éléments (avec data-idem-id) empilés sous le point, du plus haut au plus bas. */',
  '  function candidatesAt(x, y){',
  '    var els = document.elementsFromPoint(x, y) || [];',
  '    var out = [];',
  '    for (var i=0;i<els.length;i++){',
  '      var m = els[i].closest ? els[i].closest("[" + ATTR + "]") : null;',
  '      if (m && out.indexOf(m) === -1) out.push(m);',
  '    }',
  '    return out;',
  '  }',
  '',
  '  /* Arborescence complète des éléments éditables (façon explorateur de fichiers). */',
  '  function treeChildren(el){ var out = []; var kids = el.children; for (var i=0;i<kids.length;i++) treeCollect(kids[i], out); return out; }',
  '  function treeCollect(el, out){',
  '    if (el.getAttribute && el.getAttribute(ATTR)){',
  '      out.push({ id: el.getAttribute(ATTR), tag: el.tagName.toLowerCase(), kind: classify(el), children: treeChildren(el) });',
  '      return;',
  '    }',
  '    var kids = el.children; for (var i=0;i<kids.length;i++) treeCollect(kids[i], out);',
  '  }',
  '  function buildTree(){ var roots = []; var kids = document.body.children; for (var i=0;i<kids.length;i++) treeCollect(kids[i], roots); return roots; }',
  '  function sendTree(){ try { post({ type: "TREE", nodes: buildTree() }); } catch (err) {} }',
  '',
  ...
  '    post({ type: "SELECTED", elements: arr, stack: pendingStack });',
  '    pendingStack = [];',
  '  }',
  ...
  '    var gcl = g.querySelectorAll ? g.querySelectorAll("[" + ATTR + "]") : [];',
  '    for (var gi=0; gi<gcl.length; gi++) gcl[gi].removeAttribute(ATTR);',
  ...
  '  function onMove(e){',
  '    if (!enabled) return;',
  '    if (mode === "text"){ var tm = target(e); if (!tm || !isTextEditable(tm)){ hideBox(hoverBox); return; } place(hoverBox, tm); return; }',
  '    if (drag){',
  '      if (!drag.moved){',
  ...
  '  }',
  '  function onDown(e){',
  '    if (!enabled || editing || mode !== "select") return;',
  ...
  '    if (mode === "text"){ var te = target(e); if (te) beginTextEdit(te); return; }',
  '    hideBox(hoverBox);',
  '    var additive = e.shiftKey || e.metaKey || e.ctrlKey;',
  '    if (additive){ var t0 = target(e); if (t0) select(t0, true); return; }',
  '    var cands = candidatesAt(e.clientX, e.clientY);',
  '    if (!cands.length){ select(null, false); return; }',
  '    // Re-cliquer (quasi) au même point descend d un cran dans la pile.',
  '    var same = (Math.abs(e.clientX - lastX) < 8 && Math.abs(e.clientY - lastY) < 8);',
  '    if (same && cands.length > 1){ cycleIndex = (cycleIndex + 1) % cands.length; }',
  '    else { cycleIndex = 0; }',
  '    lastX = e.clientX; lastY = e.clientY;',
  '    pendingStack = []; for (var i=0;i<cands.length;i++) pendingStack.push(infoLite(cands[i]));',
  '    select(cands[cycleIndex], false);',
  '  }',
  ...
  '      if (k === "z" && !e.shiftKey){ e.preventDefault(); e.stopPropagation(); post({ type: "UNDO" }); return; }',
  '      if ((k === "z" && e.shiftKey) || k === "y"){ e.preventDefault(); e.stopPropagation(); post({ type: "REDO" }); return; }',
  '    }',
  ...
  '  }',
  '',
  '  function setEnabled(v, m){',
  '    enabled = v; mode = m || "select"; ensureBase();',
  '    document.body.style.cursor = v ? (mode === "text" ? "text" : "default") : "";',
  ...
  '    if (d.type === "ENABLE_EDIT") setEnabled(!!d.enabled, d.mode);',
  '    else if (d.type === "REQUEST_TREE") sendTree();',
  ...
  '',
  '  // L\'agent est injecté AVANT le montage de React : on renvoie l\'arbre dès que',
  '  // le DOM se peuple (et à chaque changement de structure), avec un léger debounce.',
  '  var treeTimer = null;',
  '  function scheduleTree(){ if (treeTimer) return; treeTimer = setTimeout(function(){ treeTimer = null; sendTree(); }, 250); }',
  '  try { var mo = new MutationObserver(function(){ scheduleTree(); }); mo.observe(document.body, { childList: true, subtree: true }); } catch (e) {}',
  '',
