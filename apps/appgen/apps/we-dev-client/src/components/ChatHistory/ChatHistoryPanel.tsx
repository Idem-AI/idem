...
const ChatHistoryPanel: React.FC<ChatHistoryPanelProps> = ({ open, onClose }) => {
  ...
  return (
    <div
      ref={panelRef}
      className="absolute left-0 top-full mt-1 w-72 bg-surface-1 border border-[var(--glass-border)]/10 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col"
      style={{ maxHeight: '70vh' }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--glass-border)]/10">
        <span className="text-sm font-semibold text-text-primary">Historique des chats</span>
        <button
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((session) => (
            <button
              ...
              className={`w-full flex items-start justify-between gap-2 px-4 py-3 text-left hover:bg-surface-2/5 transition-colors group ${
                session.uuid === activeChatUuid ? 'bg-surface-2/10' : ''
              }`}
            >
              <div className="flex-1 min-w-0">
                <div className="text-sm text-text-primary truncate">{session.title}</div>
                <div className="text-xs text-text-tertiary mt-0.5">{formatDate(session.updatedAt)}</div>
              </div>
              <button
                onClick={(e) => handleDeleteSession(e, session.uuid)}
                className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-surface-3/10 text-text-tertiary hover:text-red-500 dark:hover:text-red-400 flex-shrink-0"
                title="Supprimer"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
