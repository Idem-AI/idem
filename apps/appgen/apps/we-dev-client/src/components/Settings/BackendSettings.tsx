    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center">
            <div
              className={`w-11 h-6 rounded-full ${hasBackend ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'} relative`}
            >
              <div
                className={`absolute top-[2px] left-[2px] bg-white border border-gray-300 dark:border-gray-600 rounded-full h-5 w-5 transition-transform ${hasBackend ? 'translate-x-full' : ''}`}
              ></div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center space-x-2">
                {backendConfig.languageIconUrl && (
                  <img
                    src={backendConfig.languageIconUrl}
                    alt={backendConfig.language}
                    className="w-5 h-5"
                  />
                )}
                <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                  {backendConfig.language}{' '}
                  {backendConfig.languageVersion && `(${backendConfig.languageVersion})`}
                </span>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center space-x-2">
                {backendConfig.frameworkIconUrl && (
                  <img
                    src={backendConfig.frameworkIconUrl}
                    alt={backendConfig.framework}
                    className="w-5 h-5"
                  />
                )}
                <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                  {backendConfig.framework}{' '}
                  {backendConfig.frameworkVersion && `(${backendConfig.frameworkVersion})`}
                </span>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center space-x-2">
                {backendConfig.apiIconUrl && (
                  <img
                    src={backendConfig.apiIconUrl}
                    alt={backendConfig.apiType}
                    className="w-5 h-5"
                  />
                )}
                <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                  {backendConfig.apiType}{' '}
                  {backendConfig.apiVersion && `(${backendConfig.apiVersion})`}
                </span>
              </div>
            </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center space-x-2">
                  {backendConfig.ormIconUrl && (
                    <img
                      src={backendConfig.ormIconUrl}
                      alt={backendConfig.orm}
                      className="w-5 h-5"
                    />
                  )}
                  <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                    {backendConfig.orm}{' '}
                    {backendConfig.ormVersion && `(${backendConfig.ormVersion})`}
                  </span>
                </div>
              </div>
            )}
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center">
            <div
              className={`w-11 h-6 rounded-full ${hasDatabase ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'} relative`}
            >
              <div
                className={`absolute top-[2px] left-[2px] bg-white border border-gray-300 dark:border-gray-600 rounded-full h-5 w-5 transition-transform ${hasDatabase ? 'translate-x-full' : ''}`}
              ></div>
            </div>
            <span className="ml-3 text-sm text-gray-600 dark:text-gray-400">
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center space-x-2">
                {databaseConfig.providerIconUrl && (
                  <img
                    src={databaseConfig.providerIconUrl}
                    alt={databaseConfig.provider}
                    className="w-5 h-5"
                  />
                )}
                <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                  {databaseConfig.provider}{' '}
                  {databaseConfig.version && `(${databaseConfig.version})`}
                </span>
              </div>
            </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center space-x-2">
                  {databaseConfig.ormIconUrl && (
                    <img
                      src={databaseConfig.ormIconUrl}
                      alt={databaseConfig.orm}
                      className="w-5 h-5"
                    />
                  )}
                  <span className="px-2.5 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md text-sm">
                    {databaseConfig.orm}{' '}
                    {databaseConfig.ormVersion && `(${databaseConfig.ormVersion})`}
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-4">
          {Object.entries(developmentConfig.projectConfig || {}).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between">
              ...
              <div
                className={`w-8 h-4 rounded-full ${value ? 'bg-blue-600' : 'bg-gray-200 dark:bg-gray-700'} relative`}
              >
                <div
                  className={`absolute top-[2px] left-[2px] bg-white border border-gray-300 dark:border-gray-600 rounded-full h-3 w-3 transition-transform ${value ? 'translate-x-4' : ''}`}
                ></div>
              </div>
            </div>
          ))}
