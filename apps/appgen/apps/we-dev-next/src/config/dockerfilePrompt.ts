 ...
 * Ne génère pas de section Docker si les configs sont undefined
 */
export function generateDockerfilePrompt(projectData: ProjectModel): string {
  ...
  // Vérifier si on a des configs valides (pas undefined)
  const hasFrontend = frontend && frontend.framework && frontend.framework !== 'undefined';
  const hasBackend = backend && backend.language && backend.language !== 'undefined';
  const hasDatabase =
    database &&
    database.provider &&
  ...
  // Si aucune config valide, ne pas générer de section Docker
  ...
  // Frontend Docker instructions (seulement si framework défini)
  if (hasFrontend) {
  ...
  // Database Docker instructions (seulement si provider défini et valide)
  if (hasDatabase) {
  ...
  // Docker Compose instructions (seulement si on a des services à orchestrer)
  if (hasFrontend || hasBackend || hasDatabase) {
    dockerPrompt += `### Docker Compose File\n`;
    dockerPrompt += `Create a docker-compose.yml file that:\n`;

    const services = [];
    if (hasFrontend) services.push('frontend');
    if (hasBackend) services.push('backend');
    if (hasDatabase) services.push('database');

    dockerPrompt += `- Defines services: ${services.join(', ')}\n`;
