...
export async function streamResponse(
  ...
  try {
    console.log(`[StreamResponse] Processing ${messages.length} messages for model: ${model}`);
    console.log(`[StreamResponse] Last message role: ${messages[messages.length - 1]?.role}`);
    console.log(
      `[StreamResponse] Last message content preview: ${messages[messages.length - 1]?.content?.substring(0, 200)}...`
    );
    ...
    console.error(`[StreamResponse] Error processing messages:`, error);
    // 确保流被关闭
    stream.close();
