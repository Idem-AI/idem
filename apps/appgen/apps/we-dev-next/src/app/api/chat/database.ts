...
function resolveMySql(project: ProjectModel) {
  ...
  promptArr.push(
    'IMPORTANT: Based on the frontend code, place SQL files in the SQL folder, implement SQL statements, write database creation and table creation statements, and include some sample data to be inserted into the database'
  );
  promptArr.push(
    `IMPORTANT: Use MySQL as the database (${databaseConfig.version || 'latest'}), configure connection in backend with appropriate settings.`
  );

  if (databaseConfig.orm) {
    promptArr.push(
      `IMPORTANT: Use ${databaseConfig.orm} ${databaseConfig.ormVersion || ''} as ORM for database operations.`
    );
  }
...
function resolvePostgreSQL(project: ProjectModel) {
  ...
  promptArr.push(
    `IMPORTANT: Use PostgreSQL as the database (${databaseConfig.version || 'latest'}), configure connection in backend with appropriate settings.`
  );
...
function resolveMongoDB(project: ProjectModel) {
  ...
  promptArr.push(
    'IMPORTANT: Use MongoDB as the database, create appropriate collections and document schemas.'
  );
  promptArr.push(
    `IMPORTANT: Configure MongoDB connection (${databaseConfig.version || 'latest'}) in backend with appropriate settings.`
  );

  if (databaseConfig.orm) {
    promptArr.push(
      `IMPORTANT: Use ${databaseConfig.orm} ${databaseConfig.ormVersion || ''} as ODM for database operations.`
    );
  }
...
function resolveSQLite(project: ProjectModel) {
  ...
  promptArr.push(
    'IMPORTANT: Use SQLite as the database, create database file and tables with appropriate schema.'
  );
