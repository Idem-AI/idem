 ...
 *
 * Single source of truth for the UI theme: the `idem_theme` cookie
 * (`domain=.idem.africa` in production, host-only on localhost). Values:
 * 'light' | 'dark' | 'system'. Absent cookie => 'system' (mode-watcher's
 * default, which follows prefers-color-scheme).
 */
