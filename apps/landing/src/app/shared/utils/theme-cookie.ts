 ...
 * logic as the `idem_lang` cookie ([[locale-cookie.ts]]): `domain=.idem.africa`
 * in production, host-only on localhost.
 *
 * Values: 'light' | 'dark' | 'system'. Absent cookie ⇒ 'system' ⇒ the browser's
 * prefers-color-scheme decides. 'system' is written explicitly so a return to
 * "auto" propagates across apps and overrides a stale explicit choice.
 */
...
/** Read the shared theme cookie. Returns null when absent/invalid or on the server. */
...
/** Resolve a mode to a concrete dark/light ('system' → prefers-color-scheme). */
