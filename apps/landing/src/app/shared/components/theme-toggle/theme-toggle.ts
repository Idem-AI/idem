...
/**
 * Compact light/dark/system theme switcher. Changing the theme writes the shared
 * `idem_theme` cookie (via ThemeService), so the choice propagates to every other
 * Idem app. Token-based styling so it adapts to both themes.
 */
@Component({
  ...
  template: `
    <div
      ...
      @for (opt of options; track opt.mode) {
        <button
          type="button"
          class="px-2 py-1 rounded text-xs font-bold transition-opacity"
          [style.background]="
            theme.mode() === opt.mode
