...
Only run artisan commands inside "ideploy" container when in development.
...
- **Inside Docker**: `docker exec ideploy php artisan test` (for feature tests requiring database)
...
- Feature tests that require database must be run in the `ideploy` container
...
<x-forms.select canGate="update" :canResource="$resource" id="type" label="Type"
  >...</x-forms.select
>
<x-forms.checkbox
  instantSave
...
- **Feature Tests** (`tests/Feature/`): May use database. MUST run inside Docker with `docker exec ideploy php artisan test`
...
- Run inside Docker: `docker exec ideploy php artisan test`
- Run specific file: `docker exec ideploy php artisan test tests/Feature/ExampleTest.php`
- Filter by name: `docker exec ideploy php artisan test --filter=testName`
...
- **For Feature tests**: Use `docker exec ideploy php artisan test --filter=YourTest` (runs inside Docker)
