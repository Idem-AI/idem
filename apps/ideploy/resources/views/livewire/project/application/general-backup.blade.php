    ...
    <form wire:submit='submit' class="flex flex-col pb-32">
                        ...
                        @elseif ($application->build_pack === 'buildpacks')
                            <div class="flex flex-col gap-2">
                                ...
                                <x-forms.input 
                                    ...
                                    placeholder="docker://gcr.io/paketo-buildpacks/nodejs,docker://gcr.io/paketo-buildpacks/npm-install"
                                    helper="Comma-separated list of custom buildpacks to use. Leave empty for auto-detection."
                                    x-bind:disabled="!canUpdate" />
                                
                                <div class="flex flex-col gap-2 xl:flex-row">
                                    <x-forms.input 
                                        helper="Custom install command (via Procfile or BP_ env vars)"
                                        id="install_command" 
                                        label="Install Command (Optional)"
                                        x-bind:disabled="!canUpdate" />
                                    <x-forms.input 
                                        helper="Custom build command (via Procfile or BP_ env vars)"
                                        id="build_command" 
                                        label="Build Command (Optional)"
                                        x-bind:disabled="!canUpdate" />
                                    <x-forms.input 
                                        helper="Custom start command (will create/override Procfile)"
                                        id="start_command" 
                                        label="Start Command (Optional)"
                                        x-bind:disabled="!canUpdate" />
                                </div>
                            </div>
                            <div class="pt-1 text-xs">Cloud Native Buildpacks will auto-detect your application type and dependencies.
                                <a class="underline" href="https://buildpacks.io/" target="_blank">Buildpacks Documentation</a>
                            </div>
                        @endif
        ...
        <div class="pt-6 mt-6 border-t dark:border-coolgray-300">
            ...
            <div class="flex flex-col gap-4">
                <div class="p-4 bg-gray-50 dark:bg-coolgray-100 rounded-lg border border-gray-200 dark:border-coolgray-200">
                            ...
                            <p class="mt-1 text-base font-semibold text-gray-900 dark:text-white">
                                @if($application->idem_deploy_on_managed ?? true)
                                    <span class="inline-flex items-center">
                                        <span class="text-blue-600 dark:text-blue-400">☁️ IDEM Managed Servers</span>
                            ...
                            @if($application->idem_deploy_on_managed ?? false)
                                ...
                                @if($application->idem_server_strategy)
                                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        Strategy: <span class="font-medium">{{ ucfirst(str_replace('_', ' ', $application->idem_server_strategy)) }}</span>
                                    </p>
                                @endif
                            @endif
                        </div>
