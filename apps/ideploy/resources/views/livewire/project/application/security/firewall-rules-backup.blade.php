<div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400 mt-1">Configure custom rules to protect your application</p>
        </div>
        
        <div class="flex gap-3">
            <a href="{{ route('project.application.security.overview', $parameters) }}" 
               class="px-4 py-2 bg-[#151b2e] border border-gray-700 rounded-lg text-white text-sm hover:bg-[#1a2137] transition-colors">
                ← Back to Overview
            </a>
    ...
    {{-- Rules List --}}
    <div class="bg-[#151b2e] border border-gray-800 rounded-xl overflow-hidden">
        @if(count($rules) === 0)
            {{-- Empty State --}}
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-800">
                    @foreach($rules as $rule)
                        <tr class="hover:bg-[#1a2137] transition-colors">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-xs text-gray-400">
                                    ...
                                    @if(count($rule['conditions']) > 1)
                                        <span class="ml-1 px-1.5 py-0.5 bg-gray-800 rounded text-gray-400">{{ $rule['logical_operator'] }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-medium rounded
                                    ...
                                    @elseif($rule['action'] === 'log') bg-blue-900/30 text-blue-400
                                    @elseif($rule['action'] === 'allow') bg-green-900/30 text-green-400
                                    @else bg-yellow-900/30 text-yellow-400 @endif">
                                    {{ ucfirst($rule['action']) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-400">{{ number_format($rule['match_count']) }}</div>
                                @if($rule['last_match_at'])
                                    <div class="text-xs text-gray-600">{{ $rule['last_match_at'] }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" wire:click="toggleRule({{ $rule['id'] }})" 
                                           {{ $rule['enabled'] ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-2xl bg-[#0a0a0a] border border-gray-800 rounded-xl shadow-2xl transform transition-all" @click.away="$wire.closeCreateModal()">
                                    ...
                                    <div class="flex items-start gap-3 mb-4">
                                        ...
                                        <div class="flex-1">
                                            <select wire:model="newRule.conditions.{{ $index }}.field" class="w-full px-3 py-2 bg-[#151b2e] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                <option value="request_path">Request Path (URI)</option>
                                                <option value="uri_full">Full URI (with query string)</option>
                                                <option value="query_parameter">Query Parameter (ARGS)</option>
                                                <option value="post_body">POST Body (BODY_ARGS)</option>
                                                <option value="raw_body">Raw Body (complete)</option>
                                                <option value="header">Header</option>
                                                <option value="user_agent">User Agent</option>
                                                <option value="ip_address">IP Address</option>
                                                <option value="method">HTTP Method</option>
                                                <option value="protocol">HTTP Protocol</option>
                                                <option value="host">Host</option>
                                                <option value="cookie">Cookie</option>
                                                <option value="filenames">Uploaded Filenames</option>
                                                <option value="files_size">Upload Total Size</option>
                                            </select>
                                        </div>
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-t border-gray-800 bg-[#0f0f0f]">
                        <p class="text-xs text-gray-500">Review changes to publish the updated Firewall</p>
                        <div class="flex gap-3">
