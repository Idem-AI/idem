        ...
        @foreach([1 => 'Projet', 2 => 'Hébergement', 3 => 'Région'] as $n => $label)
            <div class="flex flex-col items-center {{ $n < 3 ? 'flex-1' : '' }}">
                <div class="flex items-center w-full">
                    @if($n > 1)
                        <div class="flex-1 h-px transition-colors duration-300 {{ $step >= $n ? 'bg-violet-500' : 'bg-gray-700' }}"></div>
                    @endif
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all duration-300
                        {{ $step > $n
                            ...
                            : ($step === $n
                                ? 'bg-violet-600 text-white ring-4 ring-violet-500/20 shadow-lg shadow-violet-500/20'
                ...
                <span class="text-[11px] font-semibold mt-2 transition-colors duration-300 {{ $step === $n ? 'text-white' : ($step > $n ? 'text-violet-400' : 'text-gray-600') }}">
                    {{ $label }}
                </span>
            </div>
        @endforeach
    ...
    @if($step === 1)
        <form wire:submit.prevent="nextStep" class="flex flex-col gap-5">
            ...
            <div>
                ...
                <textarea wire:model="description" rows="2"
                    ...
                    class="w-full px-4 py-3 bg-gray-900 border border-gray-700/80 rounded-xl text-white text-sm
                           ...
                           focus:outline-none focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/15"></textarea>
                @error('description')
            ...
            <div class="flex justify-end pt-1">
                <button type="submit"
                    class="inline-flex items-center gap-2 px-6 py-2.5 bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-violet-500/20 transition-all hover:-translate-y-px active:translate-y-0">
                    Continuer
        ...
        <div class="flex flex-col gap-3">
            ...
            <div wire:click="selectType('saas')"
                class="relative p-5 border-2 rounded-xl cursor-pointer transition-all duration-200
                    border-violet-500/60 bg-violet-500/5 hover:bg-violet-500/8 hover:border-violet-500/80">
                ...
                <div class="flex items-start gap-4 pr-28">
                    <div class="w-12 h-12 rounded-xl bg-violet-500/12 border border-violet-500/25 flex items-center justify-center shrink-0">
