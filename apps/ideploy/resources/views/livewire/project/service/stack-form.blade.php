<div x-data="{}">
<form wire:submit.prevent='submit' class="max-w-7xl pb-6">
    {{-- Hero Header --}}
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500/10 via-indigo-600/10 to-blue-600/10 border border-purple-500/20 p-8 mb-8">
        <div class="relative flex items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-white mb-1">Service Stack</h1>
                    <p class="text-sm text-gray-400">Configure your service stack and docker compose settings</p>
                </div>
            </div>
            <div class="flex gap-3">
                @can('update', $service)
                    <button type="submit" class="inner-button px-8 py-3">SAVE</button>
    ...
    <div class="rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 p-6 mb-6">
    ...
    @if ($fields->count() > 0)
    <div x-data="{ expanded: true }" class="group rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-orange-500/50 transition-all mb-6">
        <div @click="expanded = \!expanded" class="flex items-center justify-between p-6 cursor-pointer">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-lg font-bold text-white">Service Specific Configuration</h3>
                    <p class="text-sm text-gray-400">Environment variables and settings</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                @foreach ($fields as $serviceName => $field)
                    <div class="flex items-center gap-2">
                        <span class="font-bold text-white">{{ data_get($field, 'serviceName') }}</span>
                        <span class="text-gray-400">{{ data_get($field, 'name') }}</span>
