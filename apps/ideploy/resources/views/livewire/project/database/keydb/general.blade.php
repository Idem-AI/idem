<div x-data="{ activeTab: 'basic' }">
    <form wire:submit="submit" class="max-w-7xl pb-32">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-500/10 via-pink-600/10 to-rose-600/10 border border-red-500/20 p-8 mb-8">
            ...
            <div class="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div class="flex-1">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-red-500 to-pink-600 flex items-center justify-center shadow-lg shadow-red-500/30">
                            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"></path>
                            </svg>
                        </div>
                        <div>
                            <h1 class="text-3xl font-bold text-white mb-1">KeyDB Database</h1>
                ...
                @can('update', $database)
                    <button type="submit" class="group relative px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 rounded-xl font-semibold text-white shadow-lg shadow-red-500/30 hover:shadow-red-500/50 transition-all duration-300 hover:scale-105 whitespace-nowrap">
                        <div class="absolute inset-0 bg-gradient-to-r from-red-600 to-pink-700 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
        ...
        <div x-data="{ expanded: true }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-red-500/50 transition-all duration-300 mb-6">
            <div @click="expanded = !expanded" class="flex items-center justify-between p-6 cursor-pointer">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-white group-hover:text-red-400 transition-colors">Basic Information</h3>
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-forms.input label="Image" id="image" required
                        helper="For all available images, check here:<br><br><a target='_blank' href='https://hub.docker.com/r/eqalpha/keydb'>https://hub.docker.com/r/eqalpha/keydb</a>" canGate="update" :canResource="$database" />
                </div>
            </div>
        </div>
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-green-500/50 transition-all duration-300 mb-6">
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                @if ($database->started_at)
                    <x-forms.input label="Password" id="keydbPassword" type="password"
                        helper="You can only change this in the database." canGate="update" :canResource="$database" />
                @else
                    <x-forms.input label="Password" id="keydbPassword" type="password" required canGate="update" :canResource="$database" />
                @endif
            </div>
        </div>
                ...
                @if ($database->enable_ssl && $database->certificateValidUntil)
                    <div class="mb-4 p-4 rounded-lg bg-purple-500/10 border border-purple-500/30">
                        ...
                        @if (now()->gt($database->certificateValidUntil))
                            <p class="text-red-400 font-semibold">{{ $database->certificateValidUntil->format('d.m.Y H:i:s') }} - Expired</p>
