<div x-data="{ activeTab: 'basic' }">
    <form wire:submit="submit" class="max-w-7xl pb-32">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500/10 via-emerald-600/10 to-teal-600/10 border border-green-500/20 p-8 mb-8">
            ...
            <div class="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div class="flex-1">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-white mb-1">MongoDB Database</h1>
                ...
                @can('update', $database)
                    <button type="submit" class="group relative px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl font-semibold text-white shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all duration-300 hover:scale-105 whitespace-nowrap">
                        <div class="absolute inset-0 bg-gradient-to-r from-green-600 to-emerald-700 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
        ...
        <div x-data="{ expanded: true }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-green-500/50 transition-all duration-300 mb-6">
            <div @click="expanded = !expanded" class="flex items-center justify-between p-6 cursor-pointer">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-lg shadow-green-500/30 group-hover:shadow-green-500/50 transition-all duration-300 group-hover:scale-110">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-lg font-bold text-white group-hover:text-green-400 transition-colors">Basic Information</h3>
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-forms.input label="Image" id="image" required
                        helper="For all available images, check here:<br><br><a target='_blank' href='https://hub.docker.com/_/mongo'>https://hub.docker.com/_/mongo</a>" canGate="update" :canResource="$database" />
                </div>
            </div>
        </div>
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                @if ($database->started_at)
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <x-forms.input label="Username" id="mongoInitdbRootUsername" placeholder="If empty: postgres"
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <x-forms.input label="Username" id="mongoInitdbRootUsername" placeholder="If empty: postgres" canGate="update" :canResource="$database" />
                        <x-forms.input label="Password" id="mongoInitdbRootPassword" type="password" required canGate="update" :canResource="$database" />
                        <x-forms.input label="Initial Database" id="mongoInitdbDatabase"
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-cyan-500/50 transition-all duration-300 mb-6">
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                ...
                <x-forms.input label="MongoDB Connection String (internal)"
                    helper="If you change the user/password/port, this could be different. This is with the default values."
                    type="password" readonly wire:model="db_url" />
                @if ($db_url_public)
                    <x-forms.input label="MongoDB Connection String (public)"
                        helper="If you change the user/password/port, this could be different. This is with the default values."
                    ...
                    <div class="mx-2">
                        @if ($database->isExited())
                            <x-forms.select id="sslMode" label="SSL Mode" wire:model.live="sslMode"
                                ...
                                helper="Choose the SSL verification mode for MongoDB connections" canGate="update" :canResource="$database">
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-orange-500/50 transition-all duration-300 mb-6">
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                ...
                <x-forms.textarea label="Custom MongoDB Configuration" rows="10" id="mongoConf" canGate="update" :canResource="$database" />
            </div>
        </div>
            ...
            <div @click="expanded = !expanded" class="flex items-center justify-between p-6 cursor-pointer">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-400">Docker options, log drain and initialization scripts</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    @can('update', $database)
                        <x-modal-input buttonTitle="+ Add Init Script" title="New Init Script">
                            <form class="flex flex-col w-full gap-2 rounded-sm" wire:submit='save_new_init_script'>
                                ...
                                <x-forms.textarea rows="20" placeholder="CREATE DATABASE test;" id="new_content" label="Content" required />
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @empty
                            <div class="text-sm text-gray-400">No initialization scripts found.</div>
                        @endforelse
                    </div>
                </div>
            </div>
