<div x-data="{ activeTab: 'basic' }">
    <form wire:submit="submit" class="max-w-7xl pb-32">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500/10 via-pink-600/10 to-fuchsia-600/10 border border-purple-500/20 p-8 mb-8">
            ...
            <div class="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                <div class="flex-1">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center shadow-lg shadow-purple-500/30">
                            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"></path>
                            </svg>
                        </div>
                        <div>
                            <h1 class="text-3xl font-bold text-white mb-1">Dragonfly Database</h1>
                            <p class="text-sm text-gray-400">Configure your cache settings and credentials</p>
                        </div>
                    </div>
                </div>
                
                @can('update', $database)
                    <button type="submit" class="group relative px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl font-semibold text-white shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 transition-all duration-300 hover:scale-105 whitespace-nowrap">
                        <div class="absolute inset-0 bg-gradient-to-r from-purple-600 to-pink-700 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
            ...
            <div class="mb-6 p-4 rounded-xl bg-gradient-to-r from-orange-500/10 to-yellow-500/10 border border-orange-500/30">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-300 mt-1">If you change the values in the database, please sync it here, otherwise automations won't work. Changing them here will not change the values in the database.</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div x-data="{ expanded: true }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-purple-500/50 transition-all duration-300 mb-6">
            <div @click="expanded = !expanded" class="flex items-center justify-between p-6 cursor-pointer">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center shadow-lg shadow-purple-500/30 group-hover:shadow-purple-500/50 transition-all duration-300 group-hover:scale-110">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                    </div>
                    <div>
                        <h3 class="text-lg font-bold text-white group-hover:text-purple-400 transition-colors">Basic Information</h3>
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-forms.input label="Image" id="image" required
                        helper="For all available images, check here:<br><br><a target='_blank' href='https://hub.docker.com/r/docker/dragonfly'>https://hub.docker.com/r/docker/dragonfly</a>" canGate="update" :canResource="$database" />
                </div>
            </div>
        </div>
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-green-500/50 transition-all duration-300 mb-6">
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                @if ($database->started_at)
                    <x-forms.input label="Password" id="dragonflyPassword" type="password"
                        helper="You can only change this in the database." canGate="update" :canResource="$database" />
                @else
                    <x-forms.input label="Password" id="dragonflyPassword" type="password" required canGate="update" :canResource="$database" />
                @endif
            </div>
        </div>
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-cyan-500/50 transition-all duration-300 mb-6">
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                <x-forms.input placeholder="3000:6379" id="portsMappings" label="Ports Mappings"
                    helper="A comma separated list of ports you would like to map to the host system.<br><span class='inline-block font-bold dark:text-warning'>Example</span>3000:6379,3002:6380" canGate="update" :canResource="$database" />
        ...
        <div x-data="{ expanded: false }" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 hover:border-purple-500/50 transition-all duration-300 mb-6">
            <div @click="expanded = !expanded" class="flex items-center justify-between p-6 cursor-pointer">
                ...
                <div class="flex items-center gap-3">
                    @if ($database->enable_ssl && $database->certificateValidUntil)
            ...
            <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6">
                @if ($database->enable_ssl && $database->certificateValidUntil)
                    <div class="mb-4 p-4 rounded-lg bg-purple-500/10 border border-purple-500/30">
                        ...
                        @if (now()->gt($database->certificateValidUntil))
                            <p class="text-purple-400 font-semibold">{{ $database->certificateValidUntil->format('d.m.Y H:i:s') }} - Expired</p>
                        @elseif(now()->addDays(30)->gt($database->certificateValidUntil))
                            <p class="text-orange-400 font-semibold">{{ $database->certificateValidUntil->format('d.m.Y H:i:s') }} - Expiring soon</p>
                        @else
                            <p class="text-green-400 font-semibold">{{ $database->certificateValidUntil->format('d.m.Y H:i:s') }}</p>
                        @endif
                    </div>
                @endif
                ...
                <div class="mb-4">
                    @if (str($database->status)->contains('exited'))
                        <x-forms.checkbox id="enable_ssl" label="Enable SSL"
                            wire:model.live="enable_ssl" instantSave="instantSaveSSL" canGate="update" :canResource="$database" />
                    @else
                        <x-forms.checkbox id="enable_ssl" label="Enable SSL"
                            wire:model.live="enable_ssl" instantSave="instantSaveSSL" disabled
                            helper="Database should be stopped to change this settings." canGate="update" :canResource="$database" />
                    @endif
                </div>
            </div>
        </div>
...
# timeout 300"
                    ...
                    label="Custom Dragonfly Configuration" rows="10" id="dragonflyConf" canGate="update" :canResource="$database" />
            </div>
        </div>
