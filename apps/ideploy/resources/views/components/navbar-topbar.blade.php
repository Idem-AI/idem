    ...
    <div class="flex items-center gap-3">
        @auth
            ...
            @if(!$isAdmin)
                ...
                {{-- Plan Badge --}}
                <a href="{{ route('idem.subscription') }}" class="flex items-center gap-1.5 px-2.5 py-1.5 {{ $colors['bg'] }} rounded-md border {{ $colors['border'] }} hover:opacity-80 transition-opacity cursor-pointer">
                ...
                {{-- Apps Quota --}}
                <div class="flex items-center gap-2 px-2.5 py-1.5 bg-gray-800/60 rounded-md border border-gray-700/40 group relative" title="{{ $appsUsed }}/{{ $appsLimit }} apps used">
                    <svg class="w-3.5 h-3.5 {{ $appsAtLimit ? 'text-red-400' : 'text-blue-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                    </svg>
                    <div class="flex flex-col gap-0.5">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Apps</span>
                            <span class="text-[10px] font-bold {{ $appsAtLimit ? 'text-red-400' : 'text-gray-300' }}">{{ $appsUsed }}/{{ $appsLimit }}</span>
                        </div>
                        <div class="w-16 h-1 bg-gray-700 rounded-full overflow-hidden">
                            <div class="h-full rounded-full transition-all {{ $appsAtLimit ? 'bg-red-500' : 'bg-blue-500' }}" style="width: {{ $appsPercent }}%"></div>
                        </div>
                    </div>
                </div>

                {{-- Servers Quota --}}
                <div class="flex items-center gap-2 px-2.5 py-1.5 bg-gray-800/60 rounded-md border border-gray-700/40" title="{{ $serversUsed }}/{{ $serversDisplayLimit }} servers used">
                    <svg class="w-3.5 h-3.5 {{ $serversAtLimit ? 'text-red-400' : 'text-emerald-400' }} flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/>
                    </svg>
                    <div class="flex flex-col gap-0.5">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Servers</span>
                            <span class="text-[10px] font-bold {{ $serversAtLimit ? 'text-red-400' : 'text-gray-300' }}">{{ $serversUsed }}/{{ $serversDisplayLimit }}</span>
                        </div>
                        <div class="w-16 h-1 bg-gray-700 rounded-full overflow-hidden">
                            <div class="h-full rounded-full transition-all {{ $serversAtLimit ? 'bg-red-500' : 'bg-emerald-500' }}" style="width: {{ $serversPercent }}%"></div>
                        </div>
                    </div>
                </div>
            @else
