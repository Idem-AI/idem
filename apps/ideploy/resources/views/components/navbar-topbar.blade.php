...
<div class="flex items-center justify-between px-6 py-4 bg-[#0a0e1a] border-b border-gray-800/50 h-20" x-data="{}">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
            <img src="{{ asset('logo/logo_white.png') }}" alt="IDEM" class="h-8 w-auto" 
                 onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';">
            <div class="text-xl font-bold tracking-wide text-glow-primary" 
                 style="display:none; background: linear-gradient(135deg, var(--color-primary), var(--color-accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                IDEM
            </div>
    ...
    <div class="flex items-center gap-3">
        @auth
            @php
                ...
                // Plan colors - DÉFINI EN DEHORS DU IF
                $planColors = [
            ...
            @if(!$isAdmin)
                ...
                <a href="{{ route('idem.subscription') }}" 
                   class="flex items-center gap-2 px-3 py-2 {{ $colors['bg'] }} rounded-lg border {{ $colors['border'] }} hover:opacity-80 transition-opacity cursor-pointer">
                ...
                <div class="flex items-center gap-2 px-3 py-2 bg-gray-800/50 rounded-lg border border-gray-700/50">
                    ...
                    <span class="text-xs font-bold {{ $appsUsed >= $appsLimit ? 'text-red-400' : 'text-blue-400' }}">
                        {{ $appsUsed }}/{{ $appsLimit }}
                    </span>
                </div>
                ...
                <div class="flex items-center gap-2 px-3 py-2 bg-gray-800/50 rounded-lg border border-gray-700/50">
                    ...
                    <span class="text-xs font-bold {{ $serversUsed >= $serversLimit ? 'text-red-400' : 'text-green-400' }}">
                        {{ $serversUsed }}/{{ $serversLimit > 0 ? $serversLimit : 2 }}
                    </span>
                </div>
            ...
            <div x-show="open" 
                ...
                <div class="py-2">
                    <a href="{{ route('profile') }}" 
                       class="flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:bg-gray-800/50 hover:text-white transition-colors">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" 
                                class="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-400 hover:bg-gray-800/50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
