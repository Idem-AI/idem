  ...
  function terminalData() {
    return {
      ...
      handleSocketClose(event) {
        console.warn(
          `[Terminal] WebSocket connection closed. Code: ${event.code}, Reason: ${event.reason || 'No reason provided'}`
        );
      ...
      handleConnectionError(reason) {
        console.error(
          `[Terminal] Connection error: ${reason} (attempt ${this.reconnectAttempts + 1})`
        );
        this.connectionState = 'disconnected';
      ...
      scheduleReconnect() {
        ...
        console.warn(
          `[Terminal] Scheduling reconnect attempt ${this.reconnectAttempts + 1} in ${delay}ms`
        );
      ...
      resetPingTimeout() {
        ...
        this.pingTimeoutId = setTimeout(() => {
          ...
          console.warn(
            `[Terminal] Ping timeout - missed ${this.heartbeatMissed}/${this.maxHeartbeatMisses}`
          );
      ...
      resizeTerminal() {
        ...
        try {
          ...
          if (rows > 0 && cols > 0) {
            ...
            console.warn('[Terminal] Invalid calculated dimensions:', {
              rows,
              cols,
              height,
              width,
